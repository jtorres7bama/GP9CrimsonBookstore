...
window.deleteBook = async function(isbn) {
  try {
    ...
    const availableCount = copies.filter(c => c.copyStatus === 'In Store').length;
    const reservedCount = copies.filter(c => c.copyStatus === 'Reserved').length;
    const soldCount = copies.filter(c => c.copyStatus === 'Sold').length;
    const totalCopies = copies.length;

    // Fetch all order line items to check for orders
    const allOrderItemsResponse = await fetch(`${API_BASE_URL}/orderlineitems`);
    const allOrderItems = allOrderItemsResponse.ok ? await allOrderItemsResponse.json() : [];
    
    // Get copy IDs for this book
    const copyIDs = copies.map(c => c.copyID);
    
    // Find orders that reference any of these copies
    const relatedOrders = allOrderItems.filter(item => copyIDs.includes(item.copyID));
    const orderCount = relatedOrders.length;

    // Determine if deletion is safe
    const canDelete = totalCopies === 0 || (soldCount === totalCopies && orderCount === 0);
    const hasActiveInventory = availableCount > 0 || reservedCount > 0;
    const hasOrders = orderCount > 0;

    // Show detailed warning modal
    showDeleteBookWarning(book, {
      totalCopies,
      availableCount,
      reservedCount,
      soldCount,
    ...
    console.error('Error checking book for deletion:', error);
    alert('Error checking book details. Please try again.');
  }
};

// Show delete book warning modal with details
function showDeleteBookWarning(book, stats) {
  const warningHTML = `
    <div class="modal fade" id="deleteBookWarningModal" tabindex="-1" aria-labelledby="deleteBookWarningModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header ${stats.canDelete ? 'bg-warning' : 'bg-danger'} text-white">
            <h5 class="modal-title" id="deleteBookWarningModalLabel">
              ${stats.canDelete ? '⚠️ Confirm Book Deletion' : '❌ Cannot Delete Book'}
            </h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <h6>Book Information:</h6>
            <p class="mb-2"><strong>Title:</strong> ${escapeHtml(book.bookTitle)}</p>
            <p class="mb-2"><strong>ISBN:</strong> ${book.isbn}</p>
            <p class="mb-3"><strong>Course:</strong> ${escapeHtml(book.course)} | <strong>Major:</strong> ${escapeHtml(book.major)}</p>
            
            <hr>
            
            <h6>Current Status:</h6>
            <ul class="list-unstyled">
              <li><strong>Total Copies:</strong> ${stats.totalCopies}</li>
              <li class="text-success"><strong>Available (In Store):</strong> ${stats.availableCount}</li>
              <li class="text-warning"><strong>Reserved:</strong> ${stats.reservedCount}</li>
              <li class="text-danger"><strong>Sold:</strong> ${stats.soldCount}</li>
              <li class="text-info"><strong>Orders Referencing This Book:</strong> ${stats.orderCount}</li>
            </ul>

            ${!stats.canDelete ? `
              <div class="alert alert-danger">
                <strong>⚠️ This book cannot be deleted because:</strong>
                <ul class="mb-0 mt-2">
                  ${stats.hasActiveInventory ? '<li>There are available or reserved copies that must be removed first</li>' : ''}
                  ${stats.hasOrders ? '<li>There are existing orders that reference copies of this book</li>' : ''}
                </ul>
                <p class="mb-0 mt-2"><strong>To delete this book:</strong></p>
                <ul class="mb-0">
                  ${stats.hasActiveInventory ? '<li>First delete all available and reserved copies</li>' : ''}
                  ${stats.hasOrders ? '<li>Note: Books with sold copies and orders cannot be deleted to preserve order history</li>' : ''}
                </ul>
              </div>
            ` : `
              <div class="alert alert-warning">
                ...
                <ul class="mb-0 mt-2">
                  <li>The book record</li>
                  <li>All ${stats.totalCopies} book copy record(s)</li>
                  <li>All author records for this book</li>
                </ul>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
            ${stats.canDelete ? `
              <button type="button" class="btn btn-danger" onclick="confirmDeleteBook('${book.isbn}')">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                  <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/>
                  <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
                </svg>
  ...
  // Remove existing modal if any
  const existingModal = document.getElementById('deleteBookWarningModal');
  if (existingModal) {
    existingModal.remove();
  }
  ...
  // Initialize Bootstrap modal
  const modalElement = document.getElementById('deleteBookWarningModal');
  const modal = new bootstrap.Modal(modalElement);
...
async function confirmDeleteBook(isbn) {
  // Final confirmation
  if (!confirm('FINAL WARNING: This will permanently delete the book and all its copies. This action CANNOT be undone. Are you absolutely sure?')) {
    return;
  }

  try {
    // Close warning modal
    const warningModal = bootstrap.Modal.getInstance(document.getElementById('deleteBookWarningModal'));
    if (warningModal) {
      warningModal.hide();
    }

    // Fetch all copies to delete them first (required due to foreign key constraints)
    const copiesResponse = await fetch(`${API_BASE_URL}/bookcopy/book/${isbn}`);
    const copies = copiesResponse.ok ? await copiesResponse.json() : [];

    // Delete all copies first (in reverse order to handle any dependencies)
    for (const copy of copies.reverse()) {
      try {
        const deleteCopyResponse = await fetch(`${API_BASE_URL}/bookcopy/${copy.copyID}`, {
          method: 'DELETE'
        });
        if (!deleteCopyResponse.ok) {
          console.warn(`Failed to delete copy ${copy.copyID}`);
        }
      } catch (error) {
        console.error(`Error deleting copy ${copy.copyID}:`, error);
      }
    }

    // Delete all authors (required due to foreign key constraints)
    const authorsResponse = await fetch(`${API_BASE_URL}/authors/book/${isbn}`);
    const authors = authorsResponse.ok ? await authorsResponse.json() : [];
    
    for (const author of authors) {
      try {
        const deleteAuthorResponse = await fetch(`${API_BASE_URL}/authors/${author.authorID}`, {
          method: 'DELETE'
        });
        if (!deleteAuthorResponse.ok) {
          console.warn(`Failed to delete author ${author.authorID}`);
        }
      } catch (error) {
        console.error(`Error deleting author ${author.authorID}:`, error);
      }
    }

    // Finally, delete the book
    const deleteBookResponse = await fetch(`${API_BASE_URL}/books/${isbn}`, {
      method: 'DELETE'
    });

    if (!deleteBookResponse.ok) {
      const errorData = await deleteBookResponse.json();
    ...
    alert(`Error deleting book: ${error.message || 'Please try again.'}`);
  }
}
